...
instance ToJSON a => ToJSON (TypeT a) where
  toJSON t = object ["name" .= typeName t, "constructors" .= constrs t]

data ConsT a = ConsT { constrName   :: String
                     , arguments    :: [ArgT]
                     , constrWeight :: a
                     , constrFreq   :: Maybe Int
                     } deriving (Show)
...
instance ToJSON ArgT where
  toJSON t = object ["name" .= argumentName t, "type" .= argumentType t]
...
toTypeT (t, cons) = TypeT { typeName = t, constrs = map toConsT cons }
...
data Cons a = Cons { func      :: String        -- ^ Constructor name.
                   ...
                   , frequency :: Maybe Int     -- ^ Marking parameter.
                   } deriving (Eq,Show)
...
seqTypes =
  S.elems . S.fromList . concatMap seqTypesCons . concat . M.elems . defs
...
systemType sys
  | not (isLinear sys)
  = Algebraic
  | not (isInterruptible sys)
  = Unsupported "Given rational system is not interruptible."
  | otherwise
...
edges' atomicT idx idxSeq (t, cons) = concatMap edge' $ neighbours cons
 where
  ...
  edge' (Type s) | s `S.member` atomicT = [(tidx, idx s), (idx s, tidx)]
                 | -- double edge
...
isLinear sys = all (all linear) (M.elems $ defs sys)
 where
  atomicT = atomicTypes sys
  linear cons =
    not (any isListArg $ args cons)
      && length (compoundArgs atomicT $ args cons)
      <= 1
...
atomicTypes sys = S.fromList $ map fst ts
 where
  ts = filter isAtomic' $ M.toList (defs sys)
...
structureSize :: System a -> Int
structureSize sys = withInt (annotations sys) "size" 1000
