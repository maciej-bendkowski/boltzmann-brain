...
consWeightWarn sys = concatMap nullType (M.toList $ defs sys)
 where
  nullCons typ cons
    | null (args cons) && weight cons == 0 = Just $ WarningExt ConsWeightWarn
      { consWeightType = typ
...
instance SystemWarn ConsWeightWarn where
  report warn =
    "Found a constructor " ++ cons' ++ " in type " ++ typ' ++ " of weight 0."
   where
...
instance SystemWarn SkipWellFoundness where
  report _ =
    "Given system specifies a rational language. Skipping well-foundness check."
...
polynomialWarn sys = [ WarningExt PolynomialSpecies | polynomial sys ]
...
warningList sys =
  skipWellFoundness sys ++ consWeightWarn sys ++ polynomialWarn sys

-- | Checks whether the given input system admits no warnings.
warnings :: Bool -> Bool -> System Int -> IO ()
warnings werror force sys = unless force (checkWarns werror $ warningList sys)
